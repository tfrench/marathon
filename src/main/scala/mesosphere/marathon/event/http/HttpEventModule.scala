...
class HttpEventModule(httpEventConfiguration: HttpEventConfiguration) extends AbstractModule {
  ...
  @Provides
  @Singleton
  def provideCallbackUrlsStore(store: PersistentStore, registry: MetricRegistry): EntityStore[EventSubscribers] = {
    new MarathonStore[EventSubscribers](store, registry, () => new EventSubscribers(Set.empty[String]), "events:")
  }
}
