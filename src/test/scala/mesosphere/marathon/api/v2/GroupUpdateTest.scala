...
class GroupUpdateTest extends FunSuite with Matchers with GivenWhenThen {
  ...
  test("A group update can be applied to an empty group") {
    ...
    val update = GroupUpdate(PathId.empty, Set.empty[V2AppDefinition], Set(
      GroupUpdate("test".toPath, Set.empty[V2AppDefinition], Set(
  ...
  test("GroupUpdate will update a Group correctly") {
    ...
    val update = GroupUpdate(
      ...
      Set.empty[V2AppDefinition],
      Set(
        GroupUpdate("/test/group1".toPath, Set(V2AppDefinition("/test/group1/app3".toPath))),
        GroupUpdate(
          "/test/group3".toPath,
          Set.empty[V2AppDefinition],
          Set(GroupUpdate("/test/group3/sub1".toPath, Set(V2AppDefinition("/test/group3/sub1/app4".toPath))))
        )
      )
    )
